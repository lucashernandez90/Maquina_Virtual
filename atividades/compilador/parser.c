...
// Verifica se pilha de operandos está vazia
...
void executar_operacao(Programa *programa, char operador, PilhaOperandos *pilha_end) {
    ...
    if (operador == '+') {
        ...
        // Salva o primeiro operando em temp_resultado
        adicionar_instrucao(programa, "STA", temp_resultado);
...
// Processa uma expressão complexa com precedência de operadores e parênteses
void processar_expressao_complexa(Programa *programa, const char *var_dest, const char *expressao) {
    ...
    for (int i = 0; expr_limpa[i]; i++) {
        ...
        // Se for operador ou parêntese
            ...
            // Se havia um termo sendo construído, processar
            if (idx_termo > 0) {
                termo[idx_termo] = '\0';
            ...
            if (expr_limpa[i] == '(') {
                ...
                if (!pilha_operadores_vazia(&pilha_op)) {
                    desempilhar_operador(&pilha_op); // Remove o '('
                }
            } 
    ...
    // Processa o último termo se houver
    if (idx_termo > 0) {
        termo[idx_termo] = '\0';
        int endereco = obter_endereco_ou_valor(termo);
        empilhar_operando(&pilha_end, endereco);
    ...
    // O resultado final está no topo da pilha de operandos
...
// Processa uma atribuição simples como A = 5
...
// Processa uma expressão simples (sem precedência de operadores)
void processar_expressao_simples(Programa *programa, const char *var_dest, const char *expressao) {
    ...
    *q = '\0';
    ...
    char operador_anterior = '+';  // inicializa com adição
    ...
    for (p = expr_copia; *p; p++) {
        if (*p == '+' || *p == '-' || *p == '*' || *p == '/') {
            ...
            if (primeiro_termo) {
                ...
                // Para os termos seguintes, aplicamos o operador
                int end = obter_endereco_ou_valor(termo);
...
// Processa uma atribuição com expressão como C = A + B
...
// Analisa o arquivo LPN e gera um programa para ser convertido em assembly
Programa *analisar_arquivo(const char *nome_arquivo) {
    ...
    while (fgets(linha, TAM_MAX_LINHA, arquivo)) {
        ...
        // Ignora cabeçalhos de programa, início e fim
        ...
        // Se não for nenhum dos casos acima, trata como comando direto
        // do formato antigo (como "carrega", "adiciona", etc)
    ...
    // Adiciona HLT se não foi adicionado
